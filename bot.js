...
client.on('message',async message => {
  ...
  if(args[2]) {
    ...
    if(credits[author].credits < balance) return message.channel.send('**:heavy_multiplication_x:| ليس معك المبلغ الكافي**');
  ...
  if(message.content.startsWith(prefix + "daily")) {
    ...
    if(mentionn) {
      ...
      message.channel.send(`**:atm: | \`${number}\`, قم بكتابة الرقم للارسال**`).then(async m => {
...
client.on('guildMemberAdd', member => {
     var guild;
    while (!guild)
        guild = client.guilds.get("450098699826102282") 
let channel = member.guild.channels.find('public ');
  if (!channel) return; 
client.channels.get("540648916794998834").send(`**شكرا لك لدخولك السيرفر ياقلبي :heart: **`) 
});
